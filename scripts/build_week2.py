...
# 3
add('Prefill and decode use the GPU differently',1.5,
    table(75,182,1450,['','Prefill','One decode step'],[
        ['New positions processed','All prompt tokens','One token per active request'],
        ['Dense linear layers','Many rows × shared weights','Few rows × shared weights'],
        ['Attention','Many queries over the prompt','One query over prior K/V'],
        ['Frequent limit','Compute for large workloads','HBM traffic at small batches'],
        ['Visible to the user','Time until the first token','Time between later tokens'],
    ],[.29,.34,.37],row_h=76,size=27)+
    text(75,691,'Shape, batch size, context length, and implementation can change the bottleneck.',27,color=MUTED)+
    takeaway('The same model can have different bottlenecks in its two phases.'),
    'Prefill and decode execute the same learned layers, but with different numbers of new positions. Dense projection and MLP weights can be reused across prompt tokens. Single-token decode at a small batch has less such reuse. These are tendencies, not rules: short prefills can be dominated by launch overhead and sufficiently batched decode can shift some operators toward compute limits. Attention and MLP can have different limits within the same pass. Time to first token in a served application also includes queueing, tokenization, and other work.',('inference','berkeley'))

# 4
add('The KV cache saves work on earlier tokens',2,
...
# 5
add('Question 1: Why does a longer context slow decode?',1,
...
# 6
add('Solution 1: The cache still has to be read',1.5,
...
# 7
add('Batching reuses weights across requests',2,
    text(75,200,'Dense linear layer: Y = XW',33,700)+
    text(75,285,'One request',29,700)+label_box(75,322,210,60,'x₁',size=30)+
    text(325,362,'×',38)+label_box(385,272,305,180,'Shared weights W',size=29)+arrow(710,362,820,362)+label_box(850,322,210,60,'y₁',size=30)+
    text(75,506,'A batch of requests',29,700)+
    ''.join(label_box(75,530+i*45,210,40,f'x{i+1}',size=25) for i in range(3))+
    text(325,610,'×',38)+label_box(385,517,305,180,'The same W',size=29)+arrow(710,610,820,610)+
    ''.join(label_box(850,530+i*45,210,40,f'y{i+1}',size=25) for i in range(3))+
    lines(1130,315,['Load a weight tile,', 'use it for more', 'than one request.'],28)+
    lines(1130,550,['K/V histories remain', 'request-specific.', 'They grow with batch.'],28,color=MUTED)+
    takeaway('More work per weight byte can improve total throughput.'),
    'Each batch row is a current token from a different request. A matrix-multiply kernel can load a weight tile and reuse it across rows. This amortizes weight traffic rather than reducing the number of model parameters. In a simple BF16 linear-layer model with small batch B relative to hidden dimensions, arithmetic intensity is roughly B FLOPs per byte. Do not infer that a complete model becomes compute-bound at one universal batch size. KV traffic remains mostly per request and memory capacity constrains viable batches.',('inference',))

# 8
add('Total throughput and per-user speed',2,
...
# 10
add('Solution 2: Batch 32 meets both objectives',1.5,
...
# 11
add('Ordinary attention materializes large intermediates',2,
...
# 12
add('FlashAttention keeps one tile near compute',3,
...
# 14
add('What FlashAttention changes',1,
...
# 15
add('CUDA Graphs reduce repeated launch overhead',2,
...
# 16
add('Experiment: sweep batch size on one GPU',2,
...
def build():
    ...
    for i,s in enumerate(slides,1):
        ...
        footer+=text(1525,873,f'{i} / 18',20,color=MUTED,anchor='end')
...
<section id="overview-panel" class="deck-overlay" hidden><div class="panel-header"><h2>18 slides · 30 minutes</h2><button data-close-overlay="true" type="button">Close</button></div><div id="overview-list"></div></section>
<section id="help-panel" class="deck-overlay" hidden><div class="panel-header"><h2>Presentation controls</h2><button data-close-overlay="true" type="button">Close</button></div><p>Arrow keys: change slides. Home / End: first / last slide. Escape: close a panel.</p><p>Use Next step inside a diagram to advance its example. Each solution follows its question. Notes includes assumptions and sources.</p><p>CSV imports remain in this browser session. Print / PDF shows completed examples.</p></section>'''
    ...
    print('Built 18 slides, 30 minutes, and speaker notes.')
