...
(() => {
  ...
  const state={generation:0,flash:0,softmax:0,paging:0};
  const max={generation:2,flash:3,softmax:2,paging:4};
  const labels={
  ...
  function update(kind){
    ({generation,flash,softmax,paging})[kind]();
  ...
  window.week2Examples={parseData,importData,pagingState,softmaxResult:()=>{
