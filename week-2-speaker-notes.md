...
D_KV is the number of KV heads times the head dimension, not necessarily the full model hidden dimension. The example can be an ordinary 32-head model with head dimension 128. Each token needs 2×32×4096×2 = 524288 bytes. At 4096 cached positions that is 2147483648 bytes, or 2.147 GB, per request; 16 requests need 34.360 GB. This is logical K/V payload only, before block tail waste, metadata, weights, workspace and runtime allocations. Different request lengths add through the sum of resident positions. We use this calculation to motivate allocation and admission, not as a hardware specification to memorize.
...
## 22. What FlashAttention changes
...
## 23. Question 2: Is paged storage enough?
...
## 24. Solution 2: The kernel must consume the block table
...
## 25. CUDA Graphs reduce repeated launch overhead
...
## 26. A prefill can delay an ongoing decode batch
...
## 27. Chunked prefill limits work between decode steps
...
## 28. Prefill–decode disaggregation
...
## 29. Experiment: sweep batch size on one GPU
...
## 30. A measurement should test a bottleneck hypothesis
