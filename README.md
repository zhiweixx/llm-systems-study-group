...
| 2 | LLM inference performance | [30-slide presentation](week-2-inference.html), [speaker notes](week-2-speaker-notes.md), [benchmark lab](week-2-lab/README.md) |
...
Week 2 is dated **September 17, 2026**, with about 33 minutes of suggested full content. It starts from first-token latency and inter-token latency, explains weight reuse and KV traffic, then develops continuous batching, PagedAttention, and FlashAttention. Online softmax is derived over three slides: the weighted average, stable running state, and rescaling. A published DistServe experiment motivates chunked prefill and prefill–decode disaggregation. The questions diagnose a decode-throughput plateau and a paged KV implementation that reconstructs dense tensors; each has an immediate solution slide. The deck ends with a take-home tiled GEMM exercise and the official Triton tutorial as its reference solution (slides 29–30). These two slides are outside the lecture timing. The optional benchmark lab produces measurements from the presenter's GPU run.

A suggested **approximately 29-minute Week 2 route** skips slides 6, 7, 26, and 27 (28.75 minutes of suggested content). It keeps the complete online-softmax derivation on slides 17–19. All 30 slides remain available for reading. The shorter route leaves 15 minutes for discussion and about 6 minutes of buffer in a 50-minute meeting. Advanced disaggregation scheduling and network placement belong to Week 4.
